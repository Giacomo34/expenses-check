...
let pendingRows = []; // rows parsed from bank statement
...
function loadSettings() {
  return {
    aiKey:    localStorage.getItem('sw_ai_key') || '',
    aiModel:  localStorage.getItem('sw_ai_model') || 'google/gemini-2.0-flash-001',
    sbUrl:    localStorage.getItem('sw_sb_url') || '',
    sbKey:    localStorage.getItem('sw_sb_key') || '',
  };
}
function saveSettings() {
  ...
  localStorage.setItem('sw_ai_key', key);
  localStorage.setItem('sw_ai_model', model);
  if (sbUrl) localStorage.setItem('sw_sb_url', sbUrl);
  if (sbKey) localStorage.setItem('sw_sb_key', sbKey);
...
function openSettings() {
  ...
  document.getElementById('ai-key-input').value = s.aiKey;
  document.getElementById('ai-model-select').value = s.aiModel;
...
function initSupabase() {
  const s = loadSettings();
  const url = s.sbUrl || (typeof SUPABASE_URL !== 'undefined' ? SUPABASE_URL : '');
  const key = s.sbKey || (typeof SUPABASE_ANON_KEY !== 'undefined' ? SUPABASE_ANON_KEY : '');
...
document.getElementById('expense-form').addEventListener('submit', async e => {
  e.preventDefault();
  if (!supabaseClient || !currentUser) { openSettings(); return; }
  ...
  const { data, error } = await supabaseClient.from('expenses').insert([newExp]).select();
  ...
  if (data?.[0]) { expenses.unshift(data[0]); updateUI(); e.target.reset(); document.getElementById('date').valueAsDate = new Date(); document.getElementById('suggestion-hint').classList.add('hidden'); showToast('Expense saved!', 'green'); }
  lucide.createIcons();
});
...
document.getElementById('edit-form').addEventListener('submit', async e => {
  ...
  const { error } = await supabaseClient.from('expenses').update(updates).eq('id', id).eq('user_id', currentUser.id);
...
async function processFile(file) {
  const s = loadSettings();
  if (!s.aiKey) { openSettings(); showToast('Please enter your Vercel AI key in Settings first.', 'red'); return; }
  ...
  try {
    const rows = await parseStatementWithAI(text, s.aiKey, s.aiModel);
...
async function parseStatementWithAI(rawText, apiKey, model) {
  ...
  const prompt = `You are a bank statement parser. Parse the following bank statement text and extract ALL expense transactions (ignore income/credits unless the user needs them).
...
async function importSelected() {
  if (!supabaseClient || !currentUser) { showToast('Connect Supabase first.', 'red'); return; }
  ...
  const { data, error } = await supabaseClient.from('expenses').insert(rows).select();
  ...
  updateUI(); closeReviewModal(); showToast(`${toImport.length} transactions imported!`, 'green');
}
...
function updateCharts() {
  ...
  Chart.defaults.color = '#4a5f7f';
  if (categoryChart) categoryChart.destroy();
  categoryChart = new Chart(document.getElementById('categoryChart').getContext('2d'), {
    type: 'doughnut',
    data: { labels: CAT_NAMES, datasets: [{ data: catData, backgroundColor: chartColors, borderWidth: 2, borderColor: '#F5F5F2', hoverOffset: 10 }] },
  ...
  weeklyChart = new Chart(document.getElementById('weeklyChart').getContext('2d'), {
    type:'bar',
    data:{ labels: last7.map(d=>new Date(d).toLocaleDateString('en-GB',{weekday:'short'})), datasets:[{ label:'Expenses', data:wkData, backgroundColor:'#0d5d1a', borderRadius:8, hoverBackgroundColor:'#0a4a15' }] },
...
function showToast(msg, type='green') {
  ...
  el.className = `fixed bottom-6 right-6 z-[70] ${bg} text-white font-poppins font-semibold px-5 py-3 rounded-xl shadow-xl text-[14px] transition-all`;
  el.textContent = msg;
